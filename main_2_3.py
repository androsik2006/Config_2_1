#!/usr/bin/env python3
"""
Инструмент визуализации графа зависимостей пакетов - Этап 3
"""

import sys
import os
import json
import urllib.request
import urllib.error
import argparse
from xml.etree import ElementTree
from collections import defaultdict, deque


class DependencyGraph:
    """Класс для представления графа зависимостей"""

    def __init__(self):
        self.graph = defaultdict(list)  # Граф: пакет -> список зависимостей
        self.visited = set()  # Посещенные узлы для DFS
        self.recursion_stack = set()  # Стек рекурсии для обнаружения циклов
        self.cycles = []  # Найденные циклы
        self.depth_info = {}  # Информация о глубине для каждого узла

    def add_dependency(self, package, dependency):
        """Добавление зависимости в граф"""
        self.graph[package].append(dependency)

    def dfs_with_depth(self, node, depth, max_depth, current_path=None):
        """
        Обход графа в глубину с ограничением по глубине

        Args:
            node: Текущий узел
            depth: Текущая глубина
            max_depth: Максимальная глубина
            current_path: Текущий путь обхода (для обнаружения циклов)
        """
        if current_path is None:
            current_path = []

        # Проверка максимальной глубины
        if depth > max_depth:
            return

        # Обнаружение циклических зависимостей
        if node in self.recursion_stack:
            cycle_start = current_path.index(node)
            cycle = current_path[cycle_start:] + [node]
            if cycle not in self.cycles:
                self.cycles.append(cycle)
            return

        if node in self.visited:
            return

        # Отмечаем узел как посещенный и добавляем в стек рекурсии
        self.visited.add(node)
        self.recursion_stack.add(node)
        current_path.append(node)

        # Сохраняем информацию о глубине
        self.depth_info[node] = min(self.depth_info.get(node, float('inf')), depth)

        # Рекурсивный обход зависимостей
        for dependency in self.graph.get(node, []):
            self.dfs_with_depth(dependency, depth + 1, max_depth, current_path.copy())

        # Убираем узел из стека рекурсии
        self.recursion_stack.remove(node)
        current_path.pop()

    def build_complete_graph(self, start_package, dependencies_data, max_depth):
        """
        Построение полного графа зависимостей

        Args:
            start_package: Начальный пакет
            dependencies_data: Данные о зависимостях
            max_depth: Максимальная глубина
        """
        # Строим граф из данных о зависимостях
        for package, deps in dependencies_data.items():
            for dep in deps:
                self.add_dependency(package, dep['name'])

        # Выполняем DFS с ограничением глубины
        self.visited.clear()
        self.recursion_stack.clear()
        self.cycles.clear()
        self.depth_info.clear()

        self.dfs_with_depth(start_package, 0, max_depth)

    def get_transitive_closure(self, start_package):
        """Получение транзитивного замыкания для начального пакета"""
        closure = set()
        queue = deque([start_package])

        while queue:
            node = queue.popleft()
            if node not in closure:
                closure.add(node)
                for dependency in self.graph.get(node, []):
                    if dependency not in closure:
                        queue.append(dependency)

        return closure

    def display_graph(self, start_package):
        """Отображение графа зависимостей"""
        print(f"\n=== ГРАФ ЗАВИСИМОСТЕЙ ДЛЯ '{start_package}' ===")

        if not self.graph:
            print("Граф пуст")
            return

        # Выводим зависимости в порядке глубины
        sorted_nodes = sorted(self.depth_info.items(), key=lambda x: x[1])

        for node, depth in sorted_nodes:
            indent = "  " * depth
            dependencies = self.graph.get(node, [])
            deps_str = ", ".join(dependencies) if dependencies else "нет"
            print(f"{indent}{node} (глубина: {depth}) -> {deps_str}")

        # Выводим информацию о циклах
        if self.cycles:
            print(f"\n=== ОБНАРУЖЕНЫ ЦИКЛИЧЕСКИЕ ЗАВИСИМОСТИ ({len(self.cycles)}) ===")
            for i, cycle in enumerate(self.cycles, 1):
                print(f"Цикл {i}: {' -> '.join(cycle)}")

        print(f"\nВсего узлов в графе: {len(self.visited)}")
        print(f"Максимальная глубина: {max(self.depth_info.values(), default=0)}")


class TestRepository:
    """Класс для работы с тестовым репозиторием"""

    def __init__(self, test_file_path):
        self.test_file_path = test_file_path
        self.dependencies_data = {}

    def load_test_dependencies(self):
        """Загрузка зависимостей из тестового файла"""
        try:
            with open(self.test_file_path, 'r', encoding='utf-8') as f:
                content = f.read().strip()

            # Парсим зависимости в формате: A->B,C; B->D; C->E,F
            dependencies = {}
            lines = content.split(';')

            for line in lines:
                line = line.strip()
                if '->' in line:
                    package, deps_str = line.split('->', 1)
                    package = package.strip()
                    dep_names = [dep.strip() for dep in deps_str.split(',')]

                    dependencies[package] = []
                    for dep_name in dep_names:
                        if dep_name:  # Игнорируем пустые зависимости
                            dependencies[package].append({
                                'name': dep_name,
                                'version': 'test',
                                'group_id': 'test',
                                'artifact_id': 'test'
                            })

            self.dependencies_data = dependencies
            return dependencies

        except Exception as e:
            raise DependencyError(f"Ошибка загрузки тестового файла: {e}")

    def get_direct_dependencies(self, package_name):
        """Получение прямых зависимостей для пакета в тестовом режиме"""
        return self.dependencies_data.get(package_name, [])


def get_user_input():
    """
    Получение параметров от пользователя через командную строку
    """
    parser = argparse.ArgumentParser(description='Анализ зависимостей Maven пакетов')

    parser.add_argument('--package', '-p',
                        help='Имя пакета в формате groupId:artifactId (например: org.springframework:spring-core)')

    parser.add_argument('--repository', '-r',
                        default='https://repo1.maven.org/maven2',
                        help='URL Maven репозитория (по умолчанию: https://repo1.maven.org/maven2)')

    parser.add_argument('--depth', '-d', type=int, default=3,
                        help='Максимальная глубина анализа (по умолчанию: 3)')

    parser.add_argument('--config', '-c',
                        help='Путь к конфигурационному файлу (опционально)')

    parser.add_argument('--test-file', '-t',
                        help='Путь к файлу с тестовыми зависимостями (для тестового режима)')

    return parser.parse_args()


def load_config_from_file(config_file):
    """
    Загрузка конфигурации из JSON файла
    """
    try:
        with open(config_file, 'r', encoding='utf-8') as f:
            return json.load(f)
    except Exception as e:
        print(f"Ошибка загрузки конфигурации из файла: {e}")
        return {}


def create_configuration(args):
    """
    Создание конфигурации на основе аргументов командной строки и файла
    """
    config = {}

    # Загружаем конфигурацию из файла, если указан
    if args.config:
        file_config = load_config_from_file(args.config)
        config.update(file_config)

    # Аргументы командной строки имеют приоритет над файлом
    if args.package:
        config['package_name'] = args.package
    if args.repository:
        config['repository_url'] = args.repository
    if args.depth:
        config['max_dependency_depth'] = args.depth
    if args.test_file:
        config['test_file_path'] = args.test_file
        config['test_repository_mode'] = True

    # Устанавливаем значения по умолчанию
    if 'repository_url' not in config:
        config['repository_url'] = 'https://repo1.maven.org/maven2'
    if 'max_dependency_depth' not in config:
        config['max_dependency_depth'] = 3
    if 'test_repository_mode' not in config:
        config['test_repository_mode'] = False

    return config


def display_configuration(config):
    """Вывод всех параметров конфигурации в формате ключ-значение"""
    print("=== КОНФИГУРАЦИЯ ПРИЛОЖЕНИЯ ===")
    for key, value in config.items():
        print(f"{key}: {value}")
    print("===============================")


def validate_configuration(config):
    """Валидация параметров конфигурации"""
    errors = []

    # В тестовом режиме проверяем наличие test_file_path
    if config.get('test_repository_mode'):
        if not config.get('test_file_path'):
            errors.append("В тестовом режиме должен быть указан путь к файлу с зависимостями")
        elif not os.path.exists(config.get('test_file_path')):
            errors.append(f"Тестовый файл не найден: {config.get('test_file_path')}")
    else:
        # Проверка имени пакета для обычного режима
        if not config.get('package_name'):
            errors.append("Имя пакета не указано")
        elif not isinstance(config['package_name'], str):
            errors.append("Имя пакета должно быть строкой")
        elif ':' not in config['package_name']:
            errors.append("Имя пакета должно быть в формате groupId:artifactId")

        # Проверка URL репозитория
        repo_url = config.get('repository_url')
        if not repo_url:
            errors.append("URL репозитория не указан")
        elif not isinstance(repo_url, str):
            errors.append("URL репозитория должен быть строкой")
        elif not (repo_url.startswith('http://') or repo_url.startswith('https://')):
            errors.append("URL репозитория должен начинаться с http:// или https://")

    # Проверка максимальной глубины анализа
    max_depth = config.get('max_dependency_depth')
    if max_depth is None:
        errors.append("Максимальная глубина анализа не указана")
    elif not isinstance(max_depth, int):
        errors.append("Максимальная глубина анализа должна быть целым числом")
    elif max_depth < 1:
        errors.append("Максимальная глубина анализа должна быть положительным числом")
    elif max_depth > 10:
        errors.append("Максимальная глубина анализа не может превышать 10")

    return errors


def parse_maven_metadata(content):
    """
    Парсинг Maven metadata для извлечения информации о последней версии
    """
    try:
        root = ElementTree.fromstring(content)
        versioning = root.find('versioning')
        if versioning is not None:
            latest = versioning.find('latest')
            if latest is not None and latest.text:
                return latest.text

            release = versioning.find('release')
            if release is not None and release.text:
                return release.text

        # Если не нашли latest/release, берем последнюю версию из versions
        versions_elem = versioning.find('versions') if versioning else None
        if versions_elem is not None:
            versions = [v.text for v in versions_elem.findall('version') if v.text]
            if versions:
                # Сортируем версии и берем последнюю
                versions.sort(key=lambda v: [int(x) for x in v.split('.') if x.isdigit()])
                return versions[-1]

        return None
    except Exception as e:
        raise DependencyError(f"Ошибка парсинга Maven metadata: {e}")


def get_package_metadata(package_name, repository_url):
    """
    Получение метаданных пакета из Maven репозитория
    """
    try:
        # Разбираем имя пакета на groupId и artifactId
        parts = package_name.split(':')
        if len(parts) != 2:
            raise DependencyError(f"Некорректный формат имени пакета: {package_name}. Ожидается: groupId:artifactId")

        group_id, artifact_id = parts

        # Формируем URL для metadata
        group_path = group_id.replace('.', '/')
        metadata_url = f"{repository_url}/{group_path}/{artifact_id}/maven-metadata.xml"

        print(f"Запрос метаданных: {metadata_url}")

        # Загружаем metadata
        with urllib.request.urlopen(metadata_url) as response:
            content = response.read().decode('utf-8')

        version = parse_maven_metadata(content)
        if not version:
            raise DependencyError(f"Не удалось определить версию для пакета {package_name}")

        return group_id, artifact_id, version

    except urllib.error.HTTPError as e:
        if e.code == 404:
            raise DependencyError(f"Пакет {package_name} не найден в репозитории")
        else:
            raise DependencyError(f"HTTP ошибка при запросе метаданных: {e.code} {e.reason}")
    except urllib.error.URLError as e:
        raise DependencyError(f"Ошибка сети при запросе метаданных: {e.reason}")
    except Exception as e:
        raise DependencyError(f"Ошибка при получении метаданных: {e}")


def get_pom_file(group_id, artifact_id, version, repository_url):
    """
    Получение POM файла пакета
    """
    try:
        group_path = group_id.replace('.', '/')
        pom_url = f"{repository_url}/{group_path}/{artifact_id}/{version}/{artifact_id}-{version}.pom"

        print(f"Запрос POM файла: {pom_url}")

        with urllib.request.urlopen(pom_url) as response:
            content = response.read().decode('utf-8')

        return content

    except urllib.error.HTTPError as e:
        if e.code == 404:
            raise DependencyError(f"POM файл для {group_id}:{artifact_id}:{version} не найден")
        else:
            raise DependencyError(f"HTTP ошибка при запросе POM: {e.code} {e.reason}")
    except urllib.error.URLError as e:
        raise DependencyError(f"Ошибка сети при запросе POM: {e.reason}")
    except Exception as e:
        raise DependencyError(f"Ошибка при получении POM: {e}")


def parse_dependencies_from_pom(pom_content):
    """
    Извлечение прямых зависимостей из POM файла
    """
    try:
        namespaces = {'maven': 'http://maven.apache.org/POM/4.0.0'}
        root = ElementTree.fromstring(pom_content)

        dependencies = []

        # Ищем секцию dependencies
        dependencies_elem = root.find('maven:dependencies', namespaces)
        if dependencies_elem is None:
            # Пробуем без namespace
            dependencies_elem = root.find('dependencies')

        if dependencies_elem is not None:
            dependency_elems = dependencies_elem.findall('maven:dependency', namespaces)
            if not dependency_elems:
                dependency_elems = dependencies_elem.findall('dependency')

            for dep_elem in dependency_elems:
                group_id_elem = dep_elem.find('maven:groupId', namespaces)
                if group_id_elem is None:
                    group_id_elem = dep_elem.find('groupId')

                artifact_id_elem = dep_elem.find('maven:artifactId', namespaces)
                if artifact_id_elem is None:
                    artifact_id_elem = dep_elem.find('artifactId')

                version_elem = dep_elem.find('maven:version', namespaces)
                if version_elem is None:
                    version_elem = dep_elem.find('version')

                if group_id_elem is not None and artifact_id_elem is not None:
                    group_id = group_id_elem.text
                    artifact_id = artifact_id_elem.text
                    version = version_elem.text if version_elem is not None else "UNKNOWN"

                    dependency_name = f"{group_id}:{artifact_id}"
                    dependencies.append({
                        'name': dependency_name,
                        'version': version,
                        'group_id': group_id,
                        'artifact_id': artifact_id
                    })

        return dependencies

    except Exception as e:
        raise DependencyError(f"Ошибка парсинга POM файла: {e}")


def get_all_dependencies_recursive(start_package, repository_url, max_depth, current_depth=0, cache=None):
    """
    Рекурсивное получение всех зависимостей с ограничением глубины
    """
    if cache is None:
        cache = {}

    if current_depth > max_depth:
        return {}

    # Если уже получали зависимости для этого пакета, используем кэш
    if start_package in cache:
        return cache[start_package]

    print(f"Анализ зависимостей для {start_package} (глубина: {current_depth})")

    try:
        # Получаем прямые зависимости
        direct_deps = get_direct_dependencies(start_package, repository_url)
        cache[start_package] = direct_deps

        # Рекурсивно получаем зависимости для каждой зависимости
        all_deps = {start_package: direct_deps}

        for dep in direct_deps:
            dep_name = dep['name']
            if dep_name not in cache:
                sub_deps = get_all_dependencies_recursive(
                    dep_name, repository_url, max_depth, current_depth + 1, cache
                )
                all_deps.update(sub_deps)

        return all_deps

    except DependencyError as e:
        print(f"Предупреждение: не удалось получить зависимости для {start_package}: {e}")
        cache[start_package] = []
        return {start_package: []}


def get_direct_dependencies(package_name, repository_url):
    """
    Получение прямых зависимостей пакета
    """
    print(f"\n=== АНАЛИЗ ПАКЕТА: {package_name} ===")

    # Получаем метаданные пакета
    group_id, artifact_id, version = get_package_metadata(package_name, repository_url)
    print(f"Найдена версия: {version}")

    # Получаем POM файл
    pom_content = get_pom_file(group_id, artifact_id, version, repository_url)

    # Парсим зависимости
    dependencies = parse_dependencies_from_pom(pom_content)

    return dependencies


def display_dependencies(dependencies, package_name):
    """
    Вывод прямых зависимостей на экран
    """
    print(f"\n=== ПРЯМЫЕ ЗАВИСИМОСТИ ПАКЕТА '{package_name}' ===")

    if not dependencies:
        print("Прямые зависимости не найдены")
        return

    for i, dep in enumerate(dependencies, 1):
        print(f"{i}. {dep['name']} : {dep['version']}")

    print(f"\nВсего найдено зависимостей: {len(dependencies)}")


class DependencyError(Exception):
    """Класс для ошибок связанных с зависимостями"""
    pass


def create_test_file():
    """Создание тестового файла с различными случаями зависимостей"""
    test_cases = [
        "# Простой случай без циклов\nA->B,C;B->D,E;C->F;D->G;E->H;F->I",

        "# Случай с циклической зависимостью\nA->B;B->C;C->A",

        "# Множественные циклы\nA->B,C;B->D,C;D->A;E->F;F->E",

        "# Глубокая цепочка зависимостей\nA->B;B->C;C->D;D->E;E->F;F->G;G->H;H->I;I->J",

        "# Смешанный случай\nA->B,C,D;B->E,F;C->B,G;D->H;E->I;F->J;G->K;H->L"
    ]

    for i, test_case in enumerate(test_cases, 1):
        filename = f"test_case_{i}.txt"
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(test_case)
        print(f"Создан тестовый файл: {filename}")

    print("\nДля использования тестового режима:")
    print("python main.py --test-file test_case_1.txt --depth 3")


def main():
    """Основная функция приложения"""
    try:
        # Получаем параметры от пользователя
        args = get_user_input()

        # Создаем конфигурацию
        config = create_configuration(args)

        # Валидация конфигурации
        validation_errors = validate_configuration(config)
        if validation_errors:
            print("Ошибки валидации конфигурации:")
            for error in validation_errors:
                print(f"  - {error}")
            sys.exit(1)

        # Вывод конфигурации
        display_configuration(config)

        # Создаем граф зависимостей
        dependency_graph = DependencyGraph()

        if config['test_repository_mode']:
            # ТЕСТОВЫЙ РЕЖИМ
            print("\n=== РЕЖИМ ТЕСТИРОВАНИЯ ===")
            test_repo = TestRepository(config['test_file_path'])
            dependencies_data = test_repo.load_test_dependencies()

            # Определяем стартовый пакет (первый в файле)
            start_package = next(iter(dependencies_data.keys())) if dependencies_data else "A"
            print(f"Стартовый пакет: {start_package}")

            # Строим граф
            dependency_graph.build_complete_graph(start_package, dependencies_data, config['max_dependency_depth'])

        else:
            # ОБЫЧНЫЙ РЕЖИМ
            repository_url = config['repository_url']
            package_name = config['package_name']
            max_depth = config['max_dependency_depth']

            # Рекурсивно получаем все зависимости
            print(f"\n=== ПОЛУЧЕНИЕ ВСЕХ ЗАВИСИМОСТЕЙ (макс. глубина: {max_depth}) ===")
            all_dependencies = get_all_dependencies_recursive(package_name, repository_url, max_depth)

            # Строим граф
            dependency_graph.build_complete_graph(package_name, all_dependencies, max_depth)

        # Отображаем граф
        start_package = config.get('package_name', next(iter(dependency_graph.graph.keys()), 'A'))
        dependency_graph.display_graph(start_package)

        print("\nАнализ завершен успешно!")

    except DependencyError as e:
        print(f"Ошибка анализа зависимостей: {e}")
        sys.exit(1)
    except Exception as e:
        print(f"Неожиданная ошибка: {e}")
        sys.exit(1)


if __name__ == "__main__":
    # Если запуск без аргументов, создаем тестовые файлы
    if len(sys.argv) == 1:
        print("Создание тестовых файлов...")
        create_test_file()
        print("\nДля анализа реального пакета используйте:")
        print("python main.py --package org.springframework:spring-core --depth 3")
    else:
        main()
